...
const toReadGroup = (group: GroupRecord): IReadGroup => ({
  id: group.id,
  name: group.name,
...
export class GroupService {
  ...
  async getAll(): Promise<IReadGroup[]> {
    return this.store.read((state) => state.groups.filter((group) => !group.isDeleted).map(toReadGroup));
  }

  async create(payload: ICreateGroup): Promise<IReadGroup> {
    assertGroupName(payload.name);

    return this.store.mutate((state) => {
      const group = createGroupRecord(payload);
      state.groups.push(group);
      return toReadGroup(group);
    });
  }

  async update(id: string, payload: IUpdateGroup): Promise<IReadGroup> {
    assertGroupName(payload.name);
